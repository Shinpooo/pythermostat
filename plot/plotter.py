...
class Plotter:
    ...
    def plot_results(self, from_date=None, to_date=None, case=None):
        ...
        figures = [self.plot_temperatures(start, end)]#,
    ...
    def plot_temperatures(self, start, end):

        xticks, xticks_labels = self.get_ticks(start, end)

        T_indoor = self.results["T_indoor"][start:end]
        T_set = self.results["T_set"][start:end]
        T_heater = self.results["T_heater"][start:end]
        T_envelope = self.results["T_envelope"][start:end]
        T_ambient = self.results["T_ambient"][start:end]
        P_heater = self.results["P_heater"][start:end]
        ...
        ax2.set_ylabel('°C', fontsize=FONT_SIZE)
        ax2.plot(T_ambient, label="T_ambient")
        ax2.plot(T_envelope, label="T_envelope")
